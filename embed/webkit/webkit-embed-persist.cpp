 ...
 *
 *  $Id: webkit-embed-persist.cpp 7025 2007-05-10 17:39:37Z xrcalvar $
 */
...
struct WebkitEmbedPersistPrivate
{
...
GType
webkit_embed_persist_get_type (void)
{
       static GType type = 0;
        ...
        if (G_UNLIKELY (type == 0))
        {
                ...
                {
                        sizeof (WebkitEmbedPersistClass),
                        NULL, /* base_init */
                        NULL, /* base_finalize */
                        (GClassInitFunc) webkit_embed_persist_class_init,
                        NULL, /* class_finalize */
                        NULL, /* class_data */
                        sizeof (WebkitEmbedPersist),
                        0,    /* n_preallocs */
                        (GInstanceInitFunc) webkit_embed_persist_init
                };
...
{
        persist->priv = WEBKIT_EMBED_PERSIST_GET_PRIVATE (persist);
...
webkit_embed_persist_finalize (GObject *object)
{
        WebkitEmbedPersist *persist = WEBKIT_EMBED_PERSIST (object);
...
impl_cancel (EphyEmbedPersist *persist)
{
	nsCOMPtr<nsIWebBrowserPersist> bpersist =
		WEBKIT_EMBED_PERSIST (persist)->priv->mPersist;
...
impl_save (EphyEmbedPersist *persist)
{
	...
	if (embed)
	{
		browser = (EphyBrowser *) _webkit_embed_get_ephy_browser (WEBKIT_EMBED(embed));
	...
	if (filename == NULL || filename[0] == '\0')
	{
		...
		EphyHeaderSniffer* sniffer = new EphyHeaderSniffer
			(webPersist, WEBKIT_EMBED_PERSIST (persist),
	...
	else
	{
		...
		rv =  InitiateMozillaDownload (DOMDocument, inURI, destFile,
					       nsnull, inURI, WEBKIT_EMBED_PERSIST (persist),
...
static GObject *
webkit_embed_persist_constructor (GType type, guint n_construct_properties,
...
webkit_embed_persist_class_init (WebkitEmbedPersistClass *klass)
{
	...
	object_class->constructor = webkit_embed_persist_constructor;
