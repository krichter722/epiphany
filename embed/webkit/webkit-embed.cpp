 ...
 *
 *  $Id: webkit-embed.cpp 7025 2007-05-10 17:39:37Z xrcalvar $
 */
...
static void webkit_embed_new_window_cb		(GtkMozEmbed *embed, 
						 ...
						 guint chrome_mask,
						 WebkitEmbed *membed);
static void webkit_embed_security_change_cb	(GtkMozEmbed *embed, 
						 gpointer request,
						 PRUint32 state,
						 WebkitEmbed *membed);
static EphyEmbedSecurityLevel webkit_embed_security_level (PRUint32 state);
...
impl_manager_do_command (EphyCommandManager *manager,
			 const char *command) 
{
	WebkitEmbedPrivate *mpriv = WEBKIT_EMBED(manager)->priv;
...
webkit_embed_get_type (void)
{
        ...
        if (G_UNLIKELY (type == 0))
        {
                ...
                {
                        sizeof (WebkitEmbedClass),
                        NULL, /* base_init */
                        NULL, /* base_finalize */
                        (GClassInitFunc) webkit_embed_class_init,
                        NULL,
                        NULL, /* class_data */
                        sizeof (WebkitEmbed),
                        0, /* n_preallocs */
                        (GInstanceInitFunc) webkit_embed_init
                };
...
{
	WebkitEmbedPrivate *mpriv = WEBKIT_EMBED (embed)->priv;

	mpriv->browser->Close ();
}

static void
webkit_embed_realize (GtkWidget *widget)
{
	WebkitEmbedPrivate *mpriv = WEBKIT_EMBED (widget)->priv;
...
static GObject *
webkit_embed_constructor (GType type, guint n_construct_properties,
...
webkit_embed_class_init (WebkitEmbedClass *klass)
{
	...
	object_class->constructor = webkit_embed_constructor;
	...
	widget_class->realize = webkit_embed_realize;
...
webkit_embed_init (WebkitEmbed *embed)
{
	...
	g_signal_connect_object (embed, "location",
				 G_CALLBACK (webkit_embed_location_changed_cb),
				 embed, (GConnectFlags) 0);
	g_signal_connect_object (embed, "net_state_all",
				 G_CALLBACK (webkit_embed_net_state_all_cb),
				 embed, (GConnectFlags) 0);
	g_signal_connect_object (embed, "dom_mouse_click",
				 G_CALLBACK (webkit_embed_dom_mouse_click_cb),
				 embed, (GConnectFlags) 0);
	g_signal_connect_object (embed, "dom_mouse_down",
				 G_CALLBACK (webkit_embed_dom_mouse_down_cb),
				 embed, (GConnectFlags) 0);
	g_signal_connect_object (embed, "dom-key-press",
				 G_CALLBACK (webkit_embed_dom_key_press_cb),
				 embed, (GConnectFlags) 0);
	g_signal_connect_object (embed, "new_window",
				 G_CALLBACK (webkit_embed_new_window_cb),
				 embed, (GConnectFlags) 0);
	g_signal_connect_object (embed, "security_change",
				 G_CALLBACK (webkit_embed_security_change_cb),
				 embed, (GConnectFlags) 0);
}

gpointer
_webkit_embed_get_ephy_browser (WebkitEmbed *embed)
{
	   ...
	   EphyEmbed *preview_embed)
{
	...
	browser = WEBKIT_EMBED(embed)->priv->browser;
	...
	if (preview_embed != NULL)
	{
		...
		pbrowser = WEBKIT_EMBED(preview_embed)->priv->browser;
		if (pbrowser != NULL)
		{
...
static gboolean
webkit_embed_get_uri_parent (WebkitEmbed *membed,
			      const char *aUri,
			      nsCString &aParent)
{
        nsresult rv;
...
{
	WebkitEmbed *membed = WEBKIT_EMBED (embed);
	char *address;
	gboolean result;
	...
	nsCString parent;
	result = webkit_embed_get_uri_parent (membed, address, parent);
	g_free (address);
...
impl_get_go_up_list (EphyEmbed *embed)
{
	...
	s = address;
	nsCString parent;
	while (webkit_embed_get_uri_parent (membed, s, parent))
	{
...
impl_go_up (EphyEmbed *embed)
{
	...
	rv = webkit_embed_get_uri_parent (membed, uri, parent_uri);
	g_free (uri);
...
{
	WebkitEmbedPrivate *spriv = WEBKIT_EMBED(source)->priv;
	WebkitEmbedPrivate *dpriv = WEBKIT_EMBED(dest)->priv;
                         ...
                         char **description)
{
	...
	if (level) *level = webkit_embed_security_level (state);
...
static void
webkit_embed_location_changed_cb (GtkMozEmbed *embed, 
				   WebkitEmbed *membed)
{
	char *location;
...
static void
update_load_state (WebkitEmbed *membed, gint state)
{
	WebkitEmbedPrivate *priv = membed->priv;
	...
	if (state & GTK_MOZ_EMBED_FLAG_RESTORING &&
	    priv->load_state == WEBKIT_EMBED_LOAD_STARTED)
	{
		priv->load_state = WEBKIT_EMBED_LOAD_LOADING;
	...
	if (state & GTK_MOZ_EMBED_FLAG_IS_NETWORK)
	{
		...
		{
			priv->load_state = WEBKIT_EMBED_LOAD_STOPPED;
		}
	}
	else if (state & GTK_MOZ_EMBED_FLAG_START &&
	         state & GTK_MOZ_EMBED_FLAG_IS_REQUEST)
	{
		if (priv->load_state == WEBKIT_EMBED_LOAD_REDIRECTING)
		{
			priv->load_state = WEBKIT_EMBED_LOAD_STARTED;
		}
		else if (priv->load_state != WEBKIT_EMBED_LOAD_LOADING)
		{
			priv->load_state = WEBKIT_EMBED_LOAD_LOADING;
			...
			g_free (address);
	         ...
	         priv->load_state == WEBKIT_EMBED_LOAD_STARTED)
	{
		priv->load_state = WEBKIT_EMBED_LOAD_REDIRECTING;
	}
}

static void
webkit_embed_net_state_all_cb (GtkMozEmbed *embed, const char *aURI,
...
static gboolean
webkit_embed_emit_mouse_signal (WebkitEmbed *embed,
				 gpointer dom_event, 
				 const char *signal_name)
{
	WebkitEmbedPrivate *mpriv = embed->priv;
...
static gboolean
webkit_embed_dom_mouse_click_cb (GtkMozEmbed *embed,
				  gpointer dom_event, 
				  WebkitEmbed *membed)
{
	return webkit_embed_emit_mouse_signal (membed, dom_event,
						"ge_dom_mouse_click");
}

static gboolean
webkit_embed_dom_mouse_down_cb (GtkMozEmbed *embed, gpointer dom_event, 
				 WebkitEmbed *membed)
{
	return webkit_embed_emit_mouse_signal (membed, dom_event,
						"ge_dom_mouse_down");
...
static gint
webkit_embed_dom_key_press_cb (GtkMozEmbed *embed,
				gpointer dom_event, 
				WebkitEmbed *membed)
{
	gint retval = FALSE;
...
EphyEmbedChrome
_webkit_embed_translate_chrome (GtkMozEmbedChromeFlags flags)
{
	static const struct
	{
		guint mozilla_flag;
...
static void
webkit_embed_new_window_cb (GtkMozEmbed *embed, 
			     GtkMozEmbed **newEmbed,
			     guint chrome_mask, 
			     WebkitEmbed *membed)
{
	...
	{
		*newEmbed = _webkit_embed_new_xul_dialog ();
		return;
	}

	mask = _webkit_embed_translate_chrome (chrome);
...
static void
webkit_embed_security_change_cb (GtkMozEmbed *embed, 
				  gpointer requestptr,
				  PRUint32 state,
				  WebkitEmbed *membed)
{
	g_signal_emit_by_name (membed, "ge_security_change",
			       webkit_embed_security_level (state));
}

static EphyEmbedSecurityLevel
webkit_embed_security_level (PRUint32 state)
{
	EphyEmbedSecurityLevel level;
		   ...
		   gpointer dummy)
{
        ...
        *retval = _webkit_embed_new_xul_dialog ();
}
...
GtkMozEmbed *
_webkit_embed_new_xul_dialog (void)
{
