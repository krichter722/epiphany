 ...
 *
 *  $Id: webkit-embed-single.cpp 7025 2007-05-10 17:39:37Z xrcalvar $
 */
...
#define WEBKIT_PROFILE_DIR  "/mozilla"
#define WEBKIT_PROFILE_NAME "epiphany"
#define WEBKIT_PROFILE_FILE "prefs.js"
...
GType
webkit_embed_single_get_type (void)
{
       static GType type = 0;
        ...
        if (G_UNLIKELY (type == 0))
        {
                ...
                {
                        sizeof (WebkitEmbedSingleClass),
                        NULL, /* base_init */
                        NULL, /* base_finalize */
                        (GClassInitFunc) webkit_embed_single_class_init,
                        NULL, /* class_finalize */
                        NULL, /* class_data */
                        sizeof (WebkitEmbedSingle),
                        0,    /* n_preallocs */
                        (GInstanceInitFunc) webkit_embed_single_init
                };
...
static gboolean
mozilla_set_default_prefs (WebkitEmbedSingle *mes)
{
...
static void 
webkit_embed_single_new_window_orphan_cb (GtkMozEmbedSingle *moz_single,
...
static void
mozilla_init_single (WebkitEmbedSingle *mes)
{	
	...
	/* get single */
        ...
        g_signal_connect (G_OBJECT (single), "new_window_orphan",
                          G_CALLBACK (webkit_embed_single_new_window_orphan_cb),
			  mes);
}
...
mozilla_init_profile (void)
{
	...
	profile_path = g_build_filename (ephy_dot_dir (), 
					 WEBKIT_PROFILE_DIR,
					 (char *) NULL);
        gtk_moz_embed_set_profile_path (profile_path, WEBKIT_PROFILE_NAME);
        g_free (profile_path);
}
...
static void
mozilla_init_observer (WebkitEmbedSingle *single)
{
	EphySingle *es;
...
static void
user_css_register (WebkitEmbedSingle *single)
{
	WebkitEmbedSinglePrivate *priv = single->priv;
...
static void
user_css_unregister (WebkitEmbedSingle *single)
{
	WebkitEmbedSinglePrivate *priv = single->priv;
...
static void
mozilla_stylesheet_init (WebkitEmbedSingle *single)
{
	WebkitEmbedSinglePrivate *priv = single->priv;
...
static void
mozilla_stylesheet_shutdown (WebkitEmbedSingle *single)
{
	WebkitEmbedSinglePrivate *priv = single->priv;
...
{
	WebkitEmbedSingle *single = WEBKIT_EMBED_SINGLE (esingle);
...
{
	WebkitEmbedSingle *single = WEBKIT_EMBED_SINGLE (object);
	WebkitEmbedSinglePrivate *priv = single->priv;
...
{
	WebkitEmbedSingle *mes = WEBKIT_EMBED_SINGLE (object);
		  ...
		  const char *features)
{
	...
	if (parent)
	{
		...
		browser = (EphyBrowser *) _webkit_embed_get_ephy_browser (WEBKIT_EMBED(parent));
			 ...
			 EphyX509Cert *cert)
{
	...
	if (!certDB) return FALSE;
        ...
        rv = mozilla_x509_cert_get_mozilla_cert (WEBKIT_X509_CERT (cert),
