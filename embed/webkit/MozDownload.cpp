 ...
 * ***** END LICENSE BLOCK *****
 *
 * $Id: MozDownload.cpp 6895 2007-02-08 16:55:11Z chpe $
 */
...
MozDownload::InitForEmbed (nsIURI *aSource, nsIURI *aTarget, const nsAString &aDisplayName,
                           ...
                           nsICancelable *aCancelable, WebkitEmbedPersist *aEmbedPersist,
                           PRInt64 aMaxSize)
{
...
#ifdef GNOME_ENABLE_DEBUG
{
	    ...
	    aStateFlags == STATE_STOP)
	{
		...
		if (mEmbedPersist)
		{
			...
			{
				webkit_embed_persist_completed (mEmbedPersist);
			}
			else
			{
				webkit_embed_persist_cancelled (mEmbedPersist);
			}
		}
...
nsresult InitiateMozillaDownload (nsIDOMDocument *domDocument, nsIURI *sourceURI,
				  ...
				  nsIURI* inOriginalURI, WebkitEmbedPersist *embedPersist,
